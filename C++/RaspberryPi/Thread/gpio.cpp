...
I2C::I2C(char Addr_device , char adapter): m_addr(Addr_device) , m_adaptaer(adapter)
{
    char path[100];
...
bool I2C::ReadNowByte(uint8_t *buffer )
...
uint8_t I2C::ReadBytes(const char ByteToRead)
{
    char buffer[10]={0};
    int ios; 
    int i;
...
bool I2C::Write3Byte(uint8_t MSB , uint8_t LSB , uint8_t length)
{
...
bool I2C::Write_Bytes(uint8_t *buffer , int length)
{
    int ios;
    int wr;
    do { ios =::ioctl(m_file , I2C_SLAVE , m_addr); if (ios == -1) { perror("ioctl from write i2c: ");}} while ( ios == -1);
    do { wr = ::write(m_file , buffer , length); if ( wr != length ) {perror("write write"); std::cout << "Data to write = " << (int) buffer[0] <<" , " << (int) buffer[1] <<std::endl;     }}while (wr != length); 
 ...
 bool I2C::i2cWrRd(__u8 *buffewr , __u8 *result , int send_length , int read_length) // this for write reg adress and read without a stop bit between write and read that mean the data train is : Start | Write | ADDR | ACK | Start ( another start in place of stop with a normal write and read request.) | Read | ACK | Stop 
...
int I2C::WriteByte(char command , char reg)
{
