...
bool I2C::Write3Byte(uint8_t MSB , uint8_t LSB , uint8_t length)
{
 ...
 bool I2C::i2cWrRd(__u8 *buffewr , __u8 *result) // this for write reg adress and read without a stop bit between write and read that mean the data train is : Start | Write | ADDR | ACK | Start ( another start in place of stop with a normal write and read request.) | Read | ACK | Stop 
